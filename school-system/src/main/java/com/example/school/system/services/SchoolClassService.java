...
public class SchoolClassService {
    ...
    public SchoolApiResponse<?> createClass(SchoolClassCreateDTO classDTO) {
        ...
        if (schoolClassRepository.existsByClassGradeAndClassStream(grade, classStream)) {

            throw new SchoolResourceExistsExceptionHandler("class already exists");
        }
        schoolClassRepository.save(toClassCreate(grade, classStream));
        return SchoolApiResponse.success("class created");
    }

    private SchoolClass toClassCreate(Integer classGrade, String schoolStream) {
        SchoolClass sClass = new SchoolClass();
        sClass.setClassGrade(classGrade);
        sClass.setClassStream(schoolStream);
        return sClass;
    }
