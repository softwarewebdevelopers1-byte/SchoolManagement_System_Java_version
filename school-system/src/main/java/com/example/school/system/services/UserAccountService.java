...
import com.example.school.system.repository.UserProfileRepository;
...
public class UserAccountService {
    ...
    public UserAccountService(UserProfileRepository userRepo, PasswordHashing passwordHashing,
    ...
    public SchoolApiResponse<?> SignUpUser(UserProfileDTO User, String token) {
    ...
    public String LoginUser(@Valid LoginUser user) {
        ...
        UserProfile userFound = userRepository.findByEmail(user.email());
    ...
    private void validateSignUp(UserProfileDTO userProfileDTO) {
        if (userRepository.existsByEmail(userProfileDTO.email())) {
            throw new UserExistsExceptionHandler("User already exists");
        }
    }

    private UserProfile toTeacher(UserProfileDTO userProfileDTO) {
