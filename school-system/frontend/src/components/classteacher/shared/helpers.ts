...
export const getSubId = (sid: any): string => {
  ...
  if (typeof sid === "object") {
    const id =
      ...
      sid._id ||
      sid.$oid ||
      (typeof sid.toString === "function" ? sid.toString() : "");
...
export const marksForStudentSubjects = (student: any, subjects: any[]) => {
  ...
  const sortedSubjects = [...subjects].sort((a, b) => {
    const aElective =
      String(a.enrollmentMode).toLowerCase() === "elective" || !!a.sharedSlotId;
    const bElective =
      String(b.enrollmentMode).toLowerCase() === "elective" || !!b.sharedSlotId;
...
export const grade = (v: number, bands: CbcGradingBand[] = []): string =>
  resolveCbcBand(v, bands).cbcBand;

export const gradePoints = (v: number, bands: CbcGradingBand[] = []): number =>
  resolveCbcBand(v, bands).points;
