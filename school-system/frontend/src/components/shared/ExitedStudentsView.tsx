...
export const ExitedStudentsView: React.FC<ExitedStudentsViewProps> = ({
  ...
  const handleDelete = async (student: ExitedStudent) => {
    const recordId = student.userId;
    if (!recordId) return;

    const ok = window.confirm(
      `Delete exited record for ${student.studentFullName}? This removes the archived education-health summary only.`,
    );
    if (!ok) return;
  ...
  return (
    <div style={{ display: "grid", gap: 16 }}>
      <div style={{ display: "grid", gap: 6 }}>
        ...
        >
          Learners who completed the configured final grade are marked completed
          and kept outside current class workflows.
        </p>
      </div>
      ...
      {message ? (
        <div
          style={{
            ...panelStyle,
            color: "var(--sText, var(--dh-success-text))",
            background: "var(--sBg, var(--dh-success-bg))",
          }}
        >
          {message}
        </div>
        ...
        <table style={{ width: "100%", borderCollapse: "collapse" }}>
          <thead>
            <tr
              style={{
                textAlign: "left",
                background: "var(--sand, var(--dh-sand))",
              }}
            >
              {[
                "Student",
            ...
            {filtered.length === 0 ? (
              ...
              filtered.map((student) => (
                <tr
                  ...
                  style={{
                    borderTop: "1px solid var(--border, var(--dh-border))",
                  }}
                >
                  ...
                  <td style={{ padding: 12 }}>{student?.phoneNumber}</td>
                  <td
                    style={{
                      padding: 12,
                      display: "flex",
                    ...
                    {allowDelete ? (
                      <button
                        style={{
                          ...
                          borderColor: "var(--dBg, var(--dh-danger-bg))",
                        }}
        ...
        >
          ...
          >
            <div
              style={{
                padding: "18px 20px",
                borderBottom: "1px solid var(--border, var(--dh-border))",
                display: "flex",
            ...
            >
              <div>
                ...
                >
                  {selected.studentAdm} | Grade {selected.classGrade}{" "}
                  {selected.classStream || ""}
                </p>
              </div>
            ...
            <div style={{ padding: 20, display: "grid", gap: 14 }}>
              ...
              >
                <div style={panelStyle}>
                  <strong>{Number(selected.totalPoints || 0)}</strong>
                  <br />
                  <span
                    style={{
                      color: "var(--textMut, var(--dh-text-muted))",
                ...
                <div style={panelStyle}>
                  <strong>{selected.examCount}</strong>
                  <br />
                  <span
                    style={{
                      color: "var(--textMut, var(--dh-text-muted))",
              ...
              <table style={{ width: "100%", borderCollapse: "collapse" }}>
                ...
                <tbody>
                  {selected?.examSummaries?.map((exam) => (
                    <tr
                      key={`${exam.year}-${exam.term}-${exam.examType}-${exam.classGrade}`}
                      style={{
                        borderTop: "1px solid var(--border, var(--dh-border))",
                      }}
                    >
                      <td style={{ padding: 10 }}>
                        T{exam.term} {exam.year} {exam.examType.toUpperCase()}
                      </td>
                      <td style={{ padding: 10 }}>
                        Grade {exam.classGrade} {exam.classStream || ""}
                      </td>
